...
$stmt = $pdo->prepare("SELECT barangay FROM users WHERE user_id = :user_id");
...
LEFT JOIN donation d ON fi.food_item = d.food_item
    AND d.drop_off_point IN (SELECT user_id FROM users WHERE barangay = :barangay)
LEFT JOIN claim c ON fi.food_item = c.food_item
    AND c.claim_point IN (SELECT user_id FROM users WHERE barangay = :barangay)
...
<html>
<head>
    <title>Available Food in My Barangay</title>
</head>
<body>
    <h2>Available Food in <?= htmlspecialchars($barangay) ?></h2>

    <?php if (count($results) === 0): ?>
        <p>No available food in your barangay.</p>
    <?php else: ?>
