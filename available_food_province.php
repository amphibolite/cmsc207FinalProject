...
$stmt = $pdo->prepare("SELECT province FROM users WHERE user_id = :user_id");
...
LEFT JOIN donation d ON fi.food_item = d.food_item
    AND d.drop_off_point IN (SELECT user_id FROM users WHERE province = :province)
LEFT JOIN claim c ON fi.food_item = c.food_item
    AND c.claim_point IN (SELECT user_id FROM users WHERE province = :province)
...
<html>
<head>
    <title>Available Food in My Province</title>
    <link rel="stylesheet" href="/assets/css/style.css">
</head>
<body>
    <h2>Available Food in <?= htmlspecialchars($province) ?></h2>

    <?php if (count($results) === 0): ?>
        <p>No available food in your province.</p>
    <?php else: ?>
